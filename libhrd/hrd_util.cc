 ...
 * control block.
 */
struct ibv_device* hrd_resolve_port_index(struct hrd_ctrl_blk *cb, int port_index)
{	
	struct ibv_device **dev_list;
	int num_devices = 0;
...
void* hrd_malloc_socket(int shm_key, int size, int socket_id)
{
	...
	if(shmid == -1) {
		switch(errno) {
			case EACCES:
				hrd_red_printf("HRD: SHM malloc error: Insufficient permissions."
					" (SHM key = %d)\n", shm_key);
				break;
			case EEXIST:
				hrd_red_printf("HRD: SHM malloc error: Already exists."
					" (SHM key = %d)\n", shm_key);
				break;
			case EINVAL:
				hrd_red_printf("HRD: SHM malloc error: SHMMAX/SHMIN mismatch."
					" (SHM key = %d, size = %d)\n", shm_key, size);
				break;
			case ENOMEM:
				hrd_red_printf("HRD: SHM malloc error: Insufficient memory."
					" (SHM key = %d, size = %d)\n", shm_key, size);
				break;
			default:
